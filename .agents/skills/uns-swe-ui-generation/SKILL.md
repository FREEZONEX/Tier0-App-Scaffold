...
5. Keep page title rows clean. A page title row may contain only the title, short description/breadcrumb/status text, and compact primary actions or toolbar buttons. Do not place KPI cards, metric strips, filters, forms, data cards, charts, Gantt boards, tables, detail panels, or summary panels inside the title row; put those in separate sections below it.
6. When the requirements mention history, audit trail, activity log, traceability, genealogy, approval record, or state-change events, prefer the `$mes-ui-patterns` `timeline` snippet over a generic stacked card list.
...
10. Ensure `/` is owned by the selected app experience. The scaffold default `_app.index.tsx` is the only intentionally blank route and is marked with `TEMPLATE_BLANK_ROUTE`; it must not survive in a finished app. Workspace apps should replace it with the real home/dashboard; station, review, monitor, kiosk, or other custom apps must redirect it to the correct entry route or move the entry to `/` without the sidebar. If the app has only one primary page, that page should own `/` directly. Do not add or keep any other blank route placeholders.
...
18. Apply `$app-i18n-copy` rules before finishing: choose one product locale, normalize shell/layout/dialog/loading/error copy into that locale, and avoid mixed-language button or accessibility labels. Only introduce an app-local message module if the requirements explicitly demand runtime multi-locale support.
19. Check request effects for stable keys, then check for placeholder copy, disconnected buttons, unused imports, text overlap, and zero-height chart containers.
20. Run available lint/build checks, or explain why they could not run.
...
- Mark required fields with `FieldLabel` / `RequiredMark` from `@/components/forms`, for example `<FieldLabel htmlFor="lot" required>Lot</FieldLabel>`. If a custom label wrapper is necessary, use `data-required="true"` on the label or `data-required-marker="true"` on the explicit marker. Do not hand-write bare `<span>*</span>` markers or rely on ad hoc `text-red-*` / `text-black` classes for required asterisks.
...
- Page title rows are not content containers. Keep them visually light and reserve them for title, description/breadcrumb/status text, and compact actions only. Content blocks such as cards, KPI strips, filters, charts, Gantt boards, and tables must start below the title row.
