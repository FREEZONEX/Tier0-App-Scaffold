...
- **mq**（`@tier0/sdk/mq`）：`Tier0MQClient` —— `subscribe`/`unsubscribe`/`publish`/`on(connect|disconnect|error)`/`disconnect`，懒连接、自动重连、断连重订阅。**broker 地址/密钥从 env 读**（`VITE_TIER0_MQTT_HOST`/`PORT`/`VITE_TIER0_API_KEY`），代码不传。
- **openapi**（`@tier0/sdk/openapi`）：`unsApi` REST —— `browse`/`search`/`read`/`write` 等。env：`VITE_TIER0_API_HOST`/`KEY`。
...
- **env-gated dev 源**：有 `VITE_TIER0_MQTT_HOST` → Tier0；无 → 复用现有 mock 喂 default 图静态数据（本地/E2E 不破）。不是 UI 切换，是 env 自动判断。
...
export function createDataSource(schema: Mimic, topics: readonly string[]): DataSource {
  const hasTier0 = !!import.meta.env.VITE_TIER0_MQTT_HOST;
...
- `Topbar` 去掉 `sourceMode`/`onSourceModeChange` props 及那段 UI；broker 显示改为 `import.meta.env.VITE_TIER0_MQTT_HOST ?? "dev (mock)"`。
...
## env
`.env.example` 补 `VITE_TIER0_MQTT_HOST`/`PORT`/`VITE_TIER0_API_KEY`/`VITE_TIER0_API_HOST`（注释说明平台注入、本地可选）。`platform-integration.md` 补这组变量。
...
## 数据流总览
```
连接：HmiPage effect → createDataSource(schema, topics)
        ├─ 有 VITE_TIER0_MQTT_HOST → createTier0Source → Tier0MQClient.subscribe(env broker)
...
- **source-factory 单测**：有/无 `VITE_TIER0_MQTT_HOST` 时返回正确源类型（用 `import.meta.env` 的测试注入或抽成可传参）。
...
- **`import.meta.env` 在 server fn / 测试**：工厂用 `import.meta.env.VITE_*`（客户端可见）；server fn（uns）里 SDK 读 `VITE_TIER0_API_*`，需确认 Vite SSR 下 env 可达——plan 阶段验证。
...
- 配 `VITE_TIER0_*` 连真 broker 时：真实订阅/发布/UNS 浏览（如有测试环境）
