...
```text
src/
  start.ts                    <- TanStack Start global request middleware (CSRF guard + gateway role/session bridge, do not modify)
  ...
  routes/                     <- Interface layer (HTTP): file names define URLs and nesting
    ...
    login.tsx                 <- Hidden auth bridge outside Shell; creates admin fallback session when needed
    api/                      <- Server routes: thin wrappers that delegate to services/
      health.ts               <- Health check
      manifest.ts             <- Role manifest (do not modify)
      auth/
        select-role.ts        <- Legacy/reserved role-selection endpoint for platform-owned surfaces (do not modify)
        me.ts                 <- Current signed-session user snapshot (do not modify)
        logout.ts             <- Session cookie logout handler (do not modify)
...
2. `src/start.ts` blocks cross-origin mutating requests and bypasses auth for public paths such as `/login`, `/api/auth/*`, `/api/health`, `/api/manifest`, and TanStack runtime/build assets
3. If the gateway role is valid, the app refreshes `mes-session` when stale and continues the same request
4. If the gateway role is unknown to `PERMISSION_MATRIX`, the app returns `403`
5. If no gateway role is present, the app falls back to the existing `mes-session`
6. If there is gateway identity but still no role and no session, the app creates an admin fallback session for preview; if no admin role exists, it redirects to `/login?from=...`
7. `/login` is a hidden auth bridge, not a primary role-picker UI
8. If there is neither gateway identity nor session, the app returns `401`
