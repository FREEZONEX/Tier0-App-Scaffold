...
Tier0 SDK SSR compatibility is both a scaffold-level Vite policy and an app-code loading policy. The current `@tier0/sdk` package ships CommonJS output, so keep `vite.config.ts` `ssr.external: ["pg", "@tier0/sdk", "mqtt"]` and load SDK values through `@/lib/tier0`, which uses server-side `createRequire`. Keep `package.json` `postinstall` and `scripts/patch-tier0-sdk.mjs`; it patches the installed SDK package for Node 22 by marking it CommonJS and removing `import.meta` from the SDK's CJS runtime files. Avoid importing SDK submodules on page/SSR initialization paths. If preview fails with `ReferenceError: exports is not defined in ES module scope` from `@tier0/sdk/openapi` or `@tier0/sdk/mq`, confirm the postinstall patch ran, do not bundle the SDK with `ssr.noExternal`, and move the SDK access behind the lazy helpers in `@/lib/tier0`. Do not replace the SDK with fallback clients or hand-written UNS/Flow/MQ fetch wrappers.
