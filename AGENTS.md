...
This branch is a SCADA/HMI product branch on top of the upstream MES scaffold. Keep upstream scaffold changes flowing into this branch, but do not merge this branch back into the template's `main` or `dev` branches. HMI product code lives primarily under `src/hmi/**`, `src/services/mimics.ts`, the root HMI route, and the local `.agents/skills/hmi-*` guidance.

**Hard Git rule for this product branch:** updates are one-way only: pull/merge `origin/main` into `codex/scada-hmi-upstream-main` when the scaffold updates, then resolve conflicts on this product branch. Never merge, rebase, cherry-pick, push, or open a PR from `codex/scada-hmi-upstream-main` back into `main` or `dev`. The template branches remain clean upstream bases; this product branch carries the customer HMI implementation.

When requirements mention Tier0 platform OpenAPI, UNS reads/writes/history/search/browse, Flow management, MQTT/MQ/WebSocket real-time data, or device command publish/subscribe, use the local `$tier0-sdk` skill if present (its `references/openapi/*` cover REST/UNS/Flow and `references/mq/*` cover MQTT/MQ). The template already includes `@tier0/sdk`; prefer the server-only lazy loaders in `@/lib/tier0` for OpenAPI/MQ code that runs behind server actions or API routes, and use dynamic imports inside client-only connection actions for browser MQTT. The SDK owns Tier0 platform authentication and connection details through the platform/runtime environment; generated applications must not create user-facing API key, OpenAPI host, MQTT host, token, credential, or integration settings pages for Tier0 SDK configuration unless the user explicitly asks for an operator-managed credential console.

Recommended Tier0 SDK call convention: import `loadTier0OpenApi`, `getTier0UnsApi`, `getTier0FlowApi`, `getTier0SystemApi`, or `loadTier0Mq` from `@/lib/tier0`, then `await` the loader inside the concrete server action that actually needs platform I/O. It is safe to top-level import these lazy helper functions in server-only code paths; do not invoke them at module top level. Do not top-level import `@tier0/sdk/openapi` or `@tier0/sdk/mq` from services, route loaders, pages, or modules that are loaded during SSR startup. Dynamic SDK loading is the expected pattern for optional operations such as UNS dispatch, Flow publish, MQTT command send, or browser-side MQTT connection setup.
...
When generating or modifying the customer's SCADA process picture — the HMI mimic schema (equipment nodes, pipe/signal edges, interlock rules, symbol type selection, layout, schema validation, persistence) from a process description, P&ID image, or equipment list — use the local `$hmi-mimic-generation` skill.

When wiring live MQTT/factory data into mimic devices — topics, bindings (map/test/testOff/invert/scale/alarms), fault derivation and alarm thresholds, stale/unknown handling, mock vs real broker selection, UNS topic browsing — use the local `$hmi-data-binding` skill.

When the customer needs a device symbol that does not exist in the HMI symbol catalog (`src/hmi/symbols/capabilities.ts`), or when refining how an existing symbol draws, use the local `$hmi-symbol-authoring` skill.

When configuring operator controls on HMI devices — action buttons that publish MQTT commands (start/stop, open/close, multi-message sequences, confirmation dialogs) or named publish presets — use the local `$hmi-device-actions` skill.

When taking a generated HMI app to production on the hosting platform — environment variables, database bootstrap, gateway auth and roles, base-path deployment, MQTT/UNS credentials, post-deploy verification — use the local `$hmi-app-delivery` skill.

When verifying an HMI canvas change actually rendered / works — symbol shapes, Inspector content, dialogs, alarm rings, real-time data, role-specific UI — and you're tempted to defer to the user because "AI can't see the canvas", use the local `$hmi-visual-selfcheck` skill (this environment CAN drive a browser via chrome-devtools and self-verify).

When an HMI picture renders but behaves wrong at runtime — a symbol won't update/change color, shows "--" or a "?" badge, an action button does nothing, alarms don't fire, or MQTT is "connected" but a topic never arrives — use the local `$hmi-runtime-troubleshooting` skill (NOT preview-runtime-stability, which is only for dev-server-won't-start).

When writing or editing safety interlock rules in a mimic schema — cross-device lock/trip/inhibit conditions, chained interlocks, any `interlocks[]` entry — use the local `$hmi-interlock-authoring` skill.

When the customer wants historical trend playback, alarm thresholds (high/low limits), or operator alarm visibility on HMI devices — and you need to know the history-data-source prerequisite (Tier0 UNS history API + archiving + env), where thresholds are authored, and how breaches surface — use the local `$hmi-monitoring-alarms` skill.
