...
```
src/
  start.ts                    ← Global request middleware: CSRF guard + Gateway-only auth + 401 fallback (DO NOT modify)
  ...
  routes/                     ← Interface layer (HTTP) — file-based, file naming controls URL + nesting
    ...
    login.tsx                 ← Hidden platform-auth error bridge — outside _app, no Shell
  ...
  components/
    ...
    forms/                    ← Safe form composition: FieldGroup, FormGrid, RecordSelect, FileUpload, LineItemSection
...
The scaffold's `src/lib/auth.ts` is the canonical example: it reads
gateway-injected headers inside a server boundary and uses
`new Headers(getRequestHeaders())` before calling `parseGatewayUser`.
...
primary role for display/backwards compatibility. Preview remains a single-role
`X-Tier0-Preview-Role` view-as context.
...
2. `src/start.ts` global middleware sees the request:
   ...
   - Public paths (`/login`, `/api/health`, `/api/manifest`, TanStack runtime/build assets) bypass auth.
   - Tier0 runtime roles authoritative? → continue with the injected complete role set. Each unknown role contributes zero permissions; known roles contribute their actions to the union.
   - Role present but unknown AND not gateway-injected (a forgeable legacy/login value)? → 403 fail closed.
   - Explicit preview runtime with no selected role? → use `ADMIN_ROLE` for this request only when available; do not create a Cookie.
   - Gateway identity with no applicable role context? → 403.
   - No gateway identity? → 401.
3. `/login` is a hidden platform-auth error bridge. It never creates an App session or renders a role picker.
4. Subsequent requests enforce role access via `requireAuth("admin")`, `requireAuth("operator")`, and action access via `can(user.roles, action)`. With multiple assigned roles, any matching role/action grants access.
...
- Use `@/components/forms` for shared form primitives:
  ...
  - `FileUpload` is the default attachment/import control. It provides drag/drop, selected-file rows, removal, accept/size/count validation feedback, and locale labels. A bare `<input type="file">` still has a safe 40px native fallback, but prefer the primitive for visible product flows.
...
>
  ...
  <FieldGroup label="附件" htmlFor="receiptFiles">
    <FileUpload
      id="receiptFiles"
      accept="image/*,.pdf"
      multiple
      ...
      hint="支持图片或 PDF，单个文件不超过 10 MB"
      labels={{
        dropPrompt: "拖放文件到此处",
        browse: "选择文件",
        selectedFiles: "已选文件",
        removeFile: "移除文件",
        rejectedFiles: "文件格式、大小或数量不符合要求。",
      }}
...
**Layout + user identity:** the parent authenticated layout route (`_app.tsx`,
`station.tsx`, `review.tsx`, or a custom authenticated layout) loads the user
via `beforeLoad` from Gateway headers and passes it to the selected layout.
Read the user from any nested page via `Route.useRouteContext()` — synchronous,
type-safe, no fetch round-trip. Do NOT add a separate `/api/auth/me` fetch in
nested pages.
...
- Logout is platform-owned. Do not create an App-local logout endpoint or clear
  the Gateway Cookie; use a platform-provided logout/navigation action when the
  host exposes one.
...
- Compose form bodies from `FieldGroup` + `FormGrid`; use `LineItemSection` for line-item editors, `RecordSelect` for business-object pickers instead of bare ID options, and `FileUpload` for attachments/imports instead of an unstyled native file control.
...
- Form & action primitives: dialog/drawer forms are composed from `FieldGroup` / `FormGrid` (with `LineItemSection` for line items), business-object pickers use `RecordSelect` or an equivalent metadata-rich selector, attachment/import fields use `FileUpload`, and recommended/automatic/rule-based/bulk actions expose basis, affected records, and before/after changes via `RecommendationAction` / `ImpactPreviewDialog` (or an equivalent visible preview + confirmation) before mutating. Non-English apps pass `metaLabels` / `labels` so primitive copy matches the app locale.
...
- Branding: the app name AND locale live in ONE place — `APP_NAME` and `APP_LOCALE` in `src/lib/app-chrome.ts`. `APP_LOCALE` drives `<html lang>` (native date inputs localize: zh-CN renders 年/月/日) and the shared dialogs' default labels — set both together. The app name lives in `APP_NAME`, consumed by the sidebar brand mark and the browser tab `<title>`. Set it to the real business app name (short: it renders on at most two lines in the sidebar; one or two words plus an optional qualifier, e.g. "研发仓 WMS", "Supplier Portal"). Leaving the `"Manufacturing App"` default raises a build advisory. Also update any used layout shell in `src/components/layouts/`. Remove or replace scaffold/default copy such as "Application", "Home", "Ready", "MES", "MES App", "MES Console", "Industrial App", "Station Console", "Review Workspace", "Workspace Home", or "Industrial application scaffold" unless those names are intentionally part of the finished product. The app icon is ONE square image asset used in BOTH places — the in-app brand mark (`APP_ICON` in `src/lib/app-chrome.ts`, an image path rendered as a square `<img>` by the Shell sidebar and station header) AND the platform (it shows in the platform's app list and app detail header). Whenever the user asks to create/change the app icon, or on any rebrand, do ALL of the following together (never just one): (1) **Generate the square icon as an image** — use the platform's available image-generation capability to create a 512×512 PNG for the app's business domain. Do not draw or synthesize the icon with Node, Canvas, SVG/HTML templates, or headless-browser screenshots; ordinary image conversion/resizing is allowed only after image generation when needed to meet the PNG constraints. If no image-generation capability is available, report App Identity closeout as blocked instead of silently substituting coded artwork. **Design spec — keep it a simple, flat brand LOGO / app-mark, NOT an illustration or scene**: ONE bold, centered symbol that reads as the domain, 1–2 solid colors on a solid rounded-square background, strong contrast, generous padding. It renders as small as ~32px in the sidebar and in the platform app list, so it MUST stay legible at that size — favor a single clear glyph over anything busy. Avoid photos, rich gradients, 3D, drop-shadows, fine detail, busy scenes, and text/wordmarks. Save the final 512×512 PNG as `public/app-icon.png` (≤2MB). (2) **Point the in-app icon at it** — set `APP_ICON = "/app-icon.png"` in `src/lib/app-chrome.ts` (replacing the default `"/app-icon.svg"` placeholder); this drives the sidebar and station brand marks. (3) **Sync the SAME file to the platform** — call the `update_app_info` tool with `icon_path: "public/app-icon.png"` (include `name` when the app name changed). The in-app asset and the platform icon must always come from the same generated file. Re-run all three whenever the app is renamed or rebranded.
- Product copy: when the user asks for i18n, localization, translation, or copy cleanup, apply the platform copy/i18n Skill if it is available. Default to a single explicit locale per app surface and keep visible shell, dialog, button, loading, error, tooltip, and accessibility labels in that locale. Only introduce a message catalog when the requirements explicitly need runtime multi-language support. Page titles should name the work; do not add standing subtitles such as "this page is used for..." or "首页只用于..." that explain UI structure instead of business state. Do not render design-system commentary or implementation notes in visible UI. Text like "FX green only for key states", "Tier0 signal green", color-token explanations, layout guidance, or component usage notes belongs in docs/comments only.
